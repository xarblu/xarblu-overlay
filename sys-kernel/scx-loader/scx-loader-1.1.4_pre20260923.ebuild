...
COMMIT=247fd055eac75a376258a274c52c8b05819657c8
...
if [[ "${PV}" == *_pre* ]]; then
	SRC_URI="https://github.com/sched-ext/scx-loader/archive/${COMMIT}.tar.gz -> ${PN}-${COMMIT::8}.tar.gz"
	S="${WORKDIR}/${PN}-${COMMIT}"
else
	SRC_URI="https://github.com/sched-ext/scx-loader/archive/refs/tags/v${PV}.tar.gz -> ${P}.tar.gz"
	KEYWORDS="~amd64"
fi
...
src_install() {
	local tool
	for tool in crates/*; do
		tool="${tool#crates/}"
		einfo "Installing ${tool}"
		dobin "$(cargo_target_dir)/${tool}"
		newdoc "crates/${tool}/README.md" "${tool}.md"
	done
