...
# ("v${PV}" if unset)
BCH_VERSION=v1.39.5-18-gb87c2a19b87b
...
src_install() {
	...
	# for the module itself only
	if use initramfs; then
		local -a files=(
			/sbin/bcachefs
			/sbin/fsck.bcachefs
			/sbin/mount.bcachefs
			"$(get_udevdir)/64-bcachefs.rules"
			"$(systemd_get_systemgeneratordir)/bcachefs-mount-generator"
		)

		insinto /usr/lib/dracut/dracut.conf.d
		newins - "11-${PN}.conf" <<<"install_items+=\" ${files[*]} \""
	fi
}
