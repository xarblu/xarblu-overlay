...
# @ECLASS: cachyos-kernel-build.eclass
# @MAINTAINER:
# Xarblu <xarblu@protonmail.com>
# @AUTHOR:
# Xarblu <xarblu@protonmail.com>
# @SUPPORTED_EAPIS: 8
# @PROVIDES: kernel-build
# @BLURB: Build mechanics for CachyOS Kernels
# @DESCRIPTION:
# This eclass provides the common logic to fetch
# and setup all required resources for building the
# CachyOS kernels via kernel-build.eclass
#
# The build must only set the appropriate patch/config/tarball
# versions and then inherit this eclass.

# @ECLASS_VARIABLE: GENTOO_PATCHSET
# @PRE_INHERIT
# @DEFAULT_UNSET
# @REQUIRED
# @DESCRIPTION:
# String containing a Gentoo patchset file name
...
# @DEFAULT_UNSET
# @DESCRIPTION:
# Bcachefs patch version from
# https://github.com/xarblu/bcachefs-patches
#
# If empty/unset a pkg_setup() hook will die
# to make users aware the requested version
...
if [[ ! ${_CACHYOS_KERNEL_BUILD_ECLASS} ]]; then
_CACHYOS_KERNEL_BUILD_ECLASS=1

# --- setup for inherited eclasses ---
...
if ver_test -le 7.3; then
	...
	die "Check scripts/min-tool-version.sh for new tool requirements"
fi
...
# llvm-r1.eclass
# migrate to -r2 once rust.eclass has
LLVM_COMPAT=( {19..23} )
LLVM_OPTIONAL=1

inherit eapi9-pipestatus toolchain-funcs flag-o-matic
inherit llvm-r1 rust kernel-build verify-sig
...
# @INTERNAL
# @DESCRIPTION:
# base linux version like 7.0

# @ECLASS_VARIABLE: KERNEL_RC
# @INTERNAL
# @DESCRIPTION:
# release candidate number if this
# is an RC

# @ECLASS_VARIABLE: KERNEL_PATCH
# @INTERNAL
# @DESCRIPTION:
# patch release version if this
# is a stable version

# @ECLASS_VARIABLE: KERNEL_REL
# @INTERNAL
# @DESCRIPTION:
# additional version added by
# a _p* suffix
...
# @ECLASS_VARIABLE: CACHY_CONFIG_P
# @INTERNAL
# @DESCRIPTION:
# P for the cachyos kernel config
CACHY_CONFIG_P="${PN}-${KERNEL_BASE}-${CACHY_CONFIG_COMMIT::8}"

# @ECLASS_VARIABLE: CACHY_PATCH_P
# @INTERNAL
# @DESCRIPTION:
# P for the cachyos kernel patches
CACHY_PATCH_P="${PN}-${KERNEL_BASE}-${CACHY_PATCH_COMMIT::8}"
...
BDEPEND="
	...
	rust? (
		...
		>=dev-util/bindgen-${BINDGEN_MIN_VER}
		${RUST_DEPEND}
	)
	debug? ( dev-util/pahole )
	scx? ( dev-util/pahole )
	verify-sig? ( >=sec-keys/openpgp-keys-cachyos-20260615 )
"
PDEPEND="
...
cachyos-kernel-build_pkg_pretend() {
...
cachyos-kernel-build_pkg_setup() {
...
cachyos-kernel-build_src_unpack() {
	use verify-sig && verify-sig_verify_detached \
		"${DISTDIR}/${CACHY_BASE_TARBALL}" \
		"${DISTDIR}/${CACHY_BASE_TARBALL}.asc"
...
cachyos-kernel-build_src_configure() {
...
cachyos-kernel-build_pkg_postinst() {
...
EXPORT_FUNCTIONS pkg_pretend pkg_setup src_unpack src_prepare src_configure pkg_postinst
