...
generate() {
	...
	DIR_NAME="kwin4_effect_$(echo "$1" || die | tr '-' '_' || die)"
...
src_compile() {
	...
	use doom && generate "doom" "Doom [Burn-My-Windows]" \
		"Melt your windows"
	use energize-a && generate "energize-a" "Energize A [Burn-My-Windows]" \
		"Beam your windows away"
	use energize-b && generate "energize-b" "Energize B [Burn-My-Windows]" \
		"Using different transporter technology results in an alternative visual effect"
	use fire && generate "fire" "Fire [Burn-My-Windows]" \
		"The classic effect inspired by Compiz"
	use glide && generate "glide" "Glide [Burn-My-Windows]" \
		"Fade the window to transparency with subtle 3D effects"
	use glitch && generate "glitch" "Glitch [Burn-My-Windows]" \
		"This effect applies some intentional graphics issues to your windows"
	use hexagon && generate "hexagon" "Hexagon [Burn-My-Windows]" \
		"With glowing lines and hexagon-shaped tiles, this effect looks very sci-fi"
	use incinerate && generate "incinerate" "Incinerate [Burn-My-Windows]" \
		"A less snappy but definitely more fancy take on the fire effect"
	use pixelate && generate "pixelate" "Pixelate [Burn-My-Windows]" \
		"Pixelate the window and randomly hide the pixels"
	use pixel-wheel && generate "pixel-wheel" "Pixel Wheel [Burn-My-Windows]" \
		"Pixelate the window and hide the pixels in a wheel-like fashion"
	use pixel-wipe && generate "pixel-wipe" "Pixel Wipe [Burn-My-Windows]" \
		"Pixelate the window and hide the pixels radially, starting from the pointer position"
	use portal && generate "portal" "Portal [Burn-My-Windows]" \
		"Transfer your windows to other locations in space and time"
	use tv && generate "tv" "TV Effect [Burn-My-Windows]" \
		"Make windows close like turning off a TV"
	use tv-glitch && generate "tv-glitch" "TV Glitch [Burn-My-Windows]" \
		"Make your windows close like a very glitchy old-school TV"
	use wisps && generate "wisps" "Wisps [Burn-My-Windows]" \
		"Let your windows be carried away to the realm of dreams by these little fairies"
}

src_install() {
	insinto /usr/share/kwin/effects/
	doins -r "${BUILD_DIR}"/kwin4_effect_*
}
