...
src_install() {
	...
	pax-mark m \
		...
		"${BUILD_DIR}"/dist/bin/${MOZ_PN} \
		"${BUILD_DIR}"/dist/bin/plugin-container

	DESTDIR="${D}" ./mach install || die

	#./mach install uses wrong lib name and tries to symlink /usr/bin/firefox to the built binary
	mv "${ED}/usr/$(get_libdir)/${MOZ_PN}" "${ED}${MOZILLA_FIVE_HOME}"
	rm -r "${ED}/usr/bin"

	# Upstream cannot ship symlink but we can (bmo#658850)
	rm "${ED}${MOZILLA_FIVE_HOME}/${MOZ_PN}-bin" || die
	dosym ${MOZ_PN} ${MOZILLA_FIVE_HOME}/${MOZ_PN}-bin
