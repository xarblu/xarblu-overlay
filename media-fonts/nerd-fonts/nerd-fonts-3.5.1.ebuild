...
# curl https://raw.githubusercontent.com/ryanoasis/nerd-fonts/v${PV}/bin/scripts/lib/fonts.json | jq --raw-output '.fonts[] | "\"\(.folderName):\(.licenseId)\""'
FONTS=(
	"0xProto:OFL-1.1-no-RFN"
	"3270:BSD-3-Clause"
	"AdwaitaMono:OFL-1.1-no-RFN"
	"Agave:OFL-1.1-no-RFN"
	"AnnotationMono:OFL-1.1-no-RFN"
...
approx_fonts_disk_reqs() {
	# all fonts combined need ~8500 MiB
...
src_install() {
	...
	for suffix in ttf otf; do
		if ( find . -name "*.${suffix}" | read -r ); then
