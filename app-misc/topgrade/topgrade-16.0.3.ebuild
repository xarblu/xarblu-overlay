...
CRATES="
	...
	itoa@1.0.11
	jetbrains-toolbox-updater@1.4.0
	js-sys@0.3.69
	json@0.12.4
	lazy_static@1.4.0
	libc@0.2.170
...
src_install() {
	...
	# shell completions
	mkdir -p "${S}/completions" || die

	# bash
	"${ED}/usr/bin/topgrade" --gen-completion bash > "${S}/completions/${PN}" || die
	dobashcomp "${S}/completions/${PN}"

	# zsh
	"${ED}/usr/bin/topgrade" --gen-completion zsh > "${S}/completions/_${PN}" || die
	dozshcomp "${S}/completions/_${PN}"

	# fish
	"${ED}/usr/bin/topgrade" --gen-completion fish > "${S}/completions/${PN}.fish" || die
	dofishcomp "${S}/completions/${PN}.fish"

	# manpage
	"${ED}/usr/bin/topgrade" --gen-manpage > "${S}/completions/${PN}.1" || die
